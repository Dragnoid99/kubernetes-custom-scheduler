...
def can_preempt(pod1, pod2, pod_qos, safety_slo):
  try:
    ...
    if pod_qos[pod1_dep] < pod_qos[pod2_dep] and pod_qos[pod2_dep] > safety_slo[
        int(pod2_obj.metadata.labels['service_class'])]:
      return True
    ...
    if int(pod1_obj.metadata.labels['service_class']) == int(
        pod2_obj.metadata.labels['service_class']
    ) and pod_qos[pod1_dep] < pod_qos[pod2_dep]:
      return True
...
def feasibility_check(pod, node, pod_name, pod_cpu, pod_mem, cpu_cap, mem_cap,
                      cpu_used, mem_used, pod_qos, safety_param, safety_slo,
                      pods_preempted):
