...
def get_all_pod_qos(slo):
    ...
    try:
      ...
      if action[i] == 'Pulling' or action[i] == 'Scheduled':
        ...
        if pod_state[pod_names[i]] != 'Running':
          deployment_pause_time[pod_deployment[pod_names[i]]] += time_difference(timestamps[i], pod_end_time[pod_names[i]])
      ...
      if action[i] == 'Killing' or action[i] == 'SandboxChanged':
        ...
        # If the pod changed it's state then update it's state and increment deployment_run_time of it's deploment
          ...
          deployment_run_time[pod_deployment[pod_names[i]]] += time_difference(timestamps[i],pod_start_time[pod_names[i]])
    ...
    try:
      if pod not in pod_deletion:
        if pod_state[pod] == 'Running':
          deployment_run_time[pod_deployment[pod]] += time_difference(datetime.datetime.now(tz = IST), pod_start_time[pod])
        else:
          deployment_pause_time[pod_deployment[pod]] += time_difference(datetime.datetime.now(tz = IST), pod_end_time[pod])

      else:
        if pod_state[pod] == 'Running':
          deployment_run_time[pod_deployment[pod]] += time_difference(pod_deletion[pod], pod_start_time[pod])

        else:
          deployment_pause_time[pod_deployment[pod]] += time_difference(pod_deletion[pod], pod_end_time[pod])
    except:
      pass
  ...
  for pod in pod_deployment:
    try:
      ...
      deployment_qos[dep] = deployment_run_time[dep]/(deployment_slo[dep]+0.000000001) - (deployment_run_time[dep] + deployment_pause_time[dep])
    except:
      pass
